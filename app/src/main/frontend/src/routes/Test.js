...
const Test = () => {
    ...
    const fetchList = async () => {
        try {
            const response = await axios.get(`http://localhost:8080/like/list/user/${user.userId}`, {
                            headers: {
    ...
    const fetchUserList = async () => {
        try {
            const response2 = await axios.get(`http://localhost:8080/like/list/${user.userId}`, {
                    headers: {
                        'Authorization': `Bearer ${user?.token}` // user에서 토큰 가져오기
                    }
            });
